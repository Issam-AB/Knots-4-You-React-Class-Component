...
function Cart({cart, removeFromCart, updateAmountRequest, total }) {
    ...
    // Adds more item from the selected product.
    function increment(product){
        updateAmountRequest(product.id,product.amount + 1);
    }

    //Remove item from the selected product.
...
const mapStateToProps = state => ({
    ...
    // Perfect place to do the Subtotal and Total caluculation.
    cart: state.cart.map(product => ({
        ...product,
        subtotal: (product.price * product.amount)
    })),
    total: 
     // 'reducer' is used when you want to get an array and reduce into just one value.
